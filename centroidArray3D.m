function [] = centroidArray3D(data,MovieNum)
...
while exist(cframefoldername)==7
    ...
    positions = loaddata.dstruct_cellCentroids(zi).positions;
...
CentroidArray = NaN(length(idx_unique),Nframes*2,Nlayers);
...
for t=1:Nframes
    ...
    % loop over all z layers for centroid positions
    for z = 1:Nlayers

    % centroid position array (x,y) where the row # is the cell ID #
    positions = loaddata.dstruct_cellCentroids(z).positions;
    ...
    CentroidArray(idxArray,2*t+[-1 0],z) = positions(idx,:);
