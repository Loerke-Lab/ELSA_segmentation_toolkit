function [ ] = ELSA_segmentCellsInTime_LS(data,MovieNum,checkInterval,tvec,zlayer,goodAnglesPerim)
...
for ti = 1:length(tlvec)
    ...
    if ~logical(mod(t,checkInterval))
        ...
        [lseeds_z,lmask_z,eflag,eflag2] = modifySeedsAndMask_LS(I(:,:,zlayer),Ifilt(:,:,zlayer),lseeds_z,lmask_z,titlestr,SEerodeMask,shrinkVal,goodAnglesPerim);
...
function [seeds,mask,eflag,eflag2] = modifySeedsAndMask_LS(I,Ifilt,seeds,mask,titlestr,SEerodeMask,shrinkVal,goodAnglesPerim)
...
while unsatisfactory
    ...
    %imshow(imgrgb)
    imshow(imoverlay(imgrgb,goodAnglesPerim));
    if nargin > 3
