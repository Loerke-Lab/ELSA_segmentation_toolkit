function []= ELSA_initialize3Dseeds_SingleLayer_LS_AngleCropped(data,t,layer,seedfilter,goodAnglesPerim)
...
[seeds(:,:,layer),mask(:,:,layer),ImageBWlabel(:,:,layer),ImageSegment(:,:,layer)] = modifySeedsAndMask_LS(Ilayer,Ilayer_Filt,seeds(:,:,layer),mask(:,:,layer),titlestr,seedfilter,goodAnglesPerim);
...
function [seeds,mask,imageBWlabel,imageSegmented] = modifySeedsAndMask_LS(I,Ifilt,seeds,mask,titlestr,seedfilter,goodAnglesPerim)
...
while unsatisfactory
    ...
    imshow(imoverlay(imgrgb,goodAnglesPerim),'InitialMagnification',magVal)
    if nargin > 3
