function [] = viewresults3D(data,imageTypeStr,framedelay,tvec,layers)
...
%           tvec: time vector specifying the frames to be analyzed
%                   (e.g. [1:20])
%           layers: the z-layers desired to be analyzed (e.g. [1:5])
...
for ti = 1:length(tvec)
    ...
    switch imageTypeStr
        ...
        case 'tracking'
            ...
            loadseg = load('ImageBWlabel.mat');
            BWlabel = loadseg.ImageBWlabel(:,:,z);
            BWlabel(BWlabel==-1) = 0;
        ...
        case 'seeds'
            ...
            if exist('seeds.mat') == 2
                seeds = load('seeds.mat').seeds(:,:,z);
                mask = load('mask.mat').mask(:,:,z);
    ...
    title(['Time frame: ',num2str(t),'Z-layer',num2str(z)],'FontSize',16)
